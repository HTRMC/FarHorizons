// PlayerAbilities - player ability flags and speed values (ECS component)
// Extracted from shared/Abilities.zig for use in ECS
...
pub const PlayerAbilities = struct {
    ...
    // Movement state (moved from Player)
    ...
    pub fn getEffectiveFlyingSpeed(self: *const Self) f32 {
        if (self.flying) {
            return if (self.sprinting)
                self.flying_speed * 2.0
            else
                self.flying_speed;
        }
        // Not flying - return walking speeds
        return if (self.sprinting) 0.026 else 0.02;
    }
};
