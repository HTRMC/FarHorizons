...
/// Bits [22:0] = x(5)|y(5)|z(5)|tex_index(8)
/// Bits [31:23] = model_index(9) — selects QuadModel for vertex positions/UVs/normal
pub fn packFaceData(x: u5, y: u5, z: u5, tex_index: u8, model_index: u9, ao: [4]u2) FaceData {
...
fn unpackFaceData(fd: FaceData) struct { x: u5, y: u5, z: u5, tex_index: u8, model_index: u9 } {
    return .{
        .x = @intCast(fd.word0 & 0x1F),
        .y = @intCast((fd.word0 >> 5) & 0x1F),
        .z = @intCast((fd.word0 >> 10) & 0x1F),
        .tex_index = @intCast((fd.word0 >> 15) & 0xFF),
        .model_index = @intCast((fd.word0 >> 23) & 0x1FF),
    };
}
...
test "packFaceData roundtrip - zero values" {
    ...
    try std.testing.expectEqual(@as(u9, 0), u.model_index);
    try std.testing.expectEqual(no_ao, unpackAo(fd));
}

test "packFaceData roundtrip - max values" {
    const fd = packFaceData(31, 31, 31, 255, 511, .{ 3, 3, 3, 3 });
    const u = unpackFaceData(fd);
    ...
    try std.testing.expectEqual(@as(u9, 511), u.model_index);
...
test "packFaceData roundtrip - typical values" {
    ...
    try std.testing.expectEqual(@as(u9, 4), u.model_index);
...
test "packFaceData - no field overlap" {
    ...
    try std.testing.expectEqual(@as(u9, 0), u_x.model_index);
    ...
    try std.testing.expectEqual(@as(u9, 0), u_t.model_index);

    const fd_m = packFaceData(0, 0, 0, 0, 511, no_ao);
    const u_m = unpackFaceData(fd_m);
    try std.testing.expectEqual(@as(u5, 0), u_m.x);
    try std.testing.expectEqual(@as(u8, 0), u_m.tex_index);
    try std.testing.expectEqual(@as(u9, 511), u_m.model_index);
...
test "packFaceData - shader unpacking matches" {
    const fd = packFaceData(15, 8, 22, 130, 165, .{ 2, 1, 3, 0 });
    const w = fd.word0;
    ...
    try std.testing.expectEqual(@as(u32, 165), (w >> 23) & 0x1FF);
    const w1 = fd.word1;
